...
export default function Main(props: PageProps) {
  const 
    ...
    // Promise for each cell GIF Animation (Only for Gifs or Json Objects)
    fetchPromise = useRef<Promise<AnimatedGIF>>(),
  ...
  */
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < props.cells; i++) {
      fetchPromise.current = fetch(gif)
        .then((res) => res.arrayBuffer())
        .then(AnimatedGIF.fromBuffer);

      fetchPromisesRef.current.push(fetchPromise.current);
    }
